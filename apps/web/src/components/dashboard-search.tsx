...
export function DashboardSearch({
  ...
  const { data: apps = [], status: appsStatus } = useAppsStatus({
    enabled: open,
  });
  const { data: containers = [], status: dockerStatus } = useDockerContainers({
    enabled: open && showDocker,
  });

  const loading =
    (appsStatus === "loading" && apps.length === 0) ||
    (showDocker && dockerStatus === "loading" && containers.length === 0);
                            ...
                            >
                              ...
                              {result.type === "admin" && (
                                <>
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm">Settings</p>
                                    <p className="font-console text-xs text-muted">
                                      /settings
                                    </p>
                                  </div>
