...
async function reverseGeocode(lat: number, lon: number, apiKey?: string | null): Promise<string> {
  if (apiKey) {
...
export async function getWeatherConfig() {
  ...
  return { useCurrentLocation: settings.weatherUseCurrentLocation };
}
...
export async function getWeather(options: WeatherOptions = {}): Promise<WeatherData | null> {
  ...
  if (settings.weatherUseCurrentLocation) {
    if (options.lat == null || options.lon == null) return null;
    lat = options.lat;
    lon = options.lon;
    city = "";
  } else {
    if (settings.weatherLat == null || settings.weatherLon == null) return null;
  ...
  if (settings.weatherProvider === "openweather" && settings.openWeatherApiKey) {
    data = await fetchOpenWeather(lat, lon, settings.openWeatherApiKey, resolvedCity);
  } else {
